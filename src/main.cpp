...
const string strMessageMagic = "Zorbacoin Signed Message:\n";
...
bool CTransaction::GetCoinAge(CTxDB& txdb, u::int64_t& nCoinAge) const
{
...
bool CBlock::GetCoinAge(u::int64_t& nCoinAge) const
{
    nCoinAge = 0;
...
bool CheckDiskSpace(u::int64_t nAdditionalBytes)
{
    u::int64_t nFreeBytesAvailable = filesystem::space(GetDataDir()).available;
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Zorbacoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Death by Pino Daniele";
        CTransaction txNew;
        txNew.nTime = 1420380141;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1420380141;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 3397341 : 3397341;
        ...
        assert(block.hashMerkleRoot == uint256("0xc471b0ecc08d29a5f42e1f9597b712d9d430a51b0dd1f1d981f3f51fb12fd956"));
...
unsigned char pchMessageStart[4] = { 0xe5, 0xa6, 0xe6, 0xc2 };
        ...
        BOOST_FOREACH(CAddress& addr, vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    u::int64_t hashAddr = addr.GetHash();
